...
module Pyr::Geo::Model
  module Cache
    ...
    included do
      ...
      self.table_name  = "pyr_geo_caches"
    ...
    module ClassMethods
      ...
      def reverse_geocode_from_lat_long(latitude, longitude, precision=:half_mile)
      ...
      def make_key(latitude, longitude, precision)
        dlat = latitude.to_f
        dlng = longitude.to_f
      ...
      def fetch(latitude, longitude, precision)
      ...
      def stash(geo_cache, latitude, longitude, precision)
        key = self.make_key(latitude, longitude, precision)
        return nil if key.nil?
